...
const row = (incident: Record<string, unknown>) => ({ id: incident.id, category: incident.category, description: incident.description, location: { label: incident.location_label, lat: incident.latitude, lng: incident.longitude }, reportCount: incident.report_count, firstReported: new Date(String(incident.first_reported)).getTime(), lastReported: new Date(String(incident.last_reported)).getTime() })
...
app.post('/api/incidents/report', async (request, response, next) => {
  const { description, location } = request.body as { description?: unknown; location?: { label?: unknown; lat?: unknown; lng?: unknown } }
  ...
  try {
    ...
    const result = existing.rowCount ? await client.query('UPDATE incidents SET report_count = report_count + 1, last_reported = now() WHERE id = $1 RETURNING *', [existing.rows[0].id]) : await client.query('INSERT INTO incidents (category, description, location_label, latitude, longitude) VALUES ($1, $2, $3, $4, $5) RETURNING *', [category, description.trim(), location.label.slice(0, 100), location.lat, location.lng])
