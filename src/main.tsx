...
function Map({ incidents, area, reportLocation, onSelect, onPick }: { incidents: Incident[]; area: Location; reportLocation: Location; onSelect: (id: string) => void; onPick: (location: Location) => void }) {
  const map = useRef<LeafletMap | null>(null)
  const layer = useRef<{ clearLayers: () => void; addLayer: (marker: unknown) => void } | null>(null)
  const pickRef = useRef(onPick)
  pickRef.current = onPick
  useEffect(() => { if (!window.L || map.current) return; const leaf = window.L.map('map', { zoomControl: false }).setView([area.lat, area.lng], 12); map.current = leaf; leaf.on('click', (event: { latlng: { lat: number; lng: number } }) => pickRef.current({ label: 'Map pin', lat: event.latlng.lat, lng: event.latlng.lng })); window.L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '© OpenStreetMap contributors' }).addTo(leaf); layer.current = window.L.layerGroup().addTo(leaf) }, [])
  useEffect(() => { if (!window.L || !map.current || !layer.current) return; map.current.setView([area.lat, area.lng]); layer.current.clearLayers(); incidents.forEach(incident => { const marker = window.L!.marker([incident.location.lat, incident.location.lng], { icon: window.L!.divIcon({ className: '', html: `<div class="marker ${severity(incident.reportCount)}"></div>`, iconSize: [35, 35], iconAnchor: [17, 17] }) }).addTo(layer.current!); marker.bindPopup(`<b>${incident.category}</b><br>${incident.description}`).on('click', () => onSelect(incident.id)) }); window.L.marker([reportLocation.lat, reportLocation.lng], { icon: window.L.divIcon({ className: '', html: '<div class="marker draft"></div>', iconSize: [28, 28], iconAnchor: [14, 14] }) }).addTo(layer.current!) }, [incidents, area, reportLocation, onSelect])
  return <div className="map-shell"><div id="map">{!window.L && <p className="map-error">Map tiles are unavailable. Your reports still work.</p>}</div><div className="map-hud"><span className="live-dot" /> Click map to place report pin</div><div className="legend"><span><i className="reported" />Reported</span><span><i className="confirmed" />Confirmed</span><span><i className="critical" />Critical</span></div></div>
}
...
function App() {
  ...
  const [reportLocation, setReportLocation] = useState(readArea)
  ...
  useEffect(() => { navigator.geolocation?.getCurrentPosition(position => { const next = { label: 'Your current area', lat: position.coords.latitude, lng: position.coords.longitude }; localStorage.setItem(AREA_KEY, JSON.stringify(next)); setArea(next); setReportLocation(next) }, undefined, { timeout: 5000 }) }, [])
  const submit = async (event: FormEvent) => { event.preventDefault(); const description = text.trim(); if (!description) return; const body = new FormData(); body.append('description', description); body.append('location', JSON.stringify(reportLocation)); if (image) body.append('image', image); const response = await fetch('/api/incidents/report', { method: 'POST', body }); if (response.ok) { setText(''); setImage(null); await load() } }
  ...
  const changeArea = () => { const label = prompt('Your area in Kochi:', area.label); if (!label) return; const next = { ...kochi, label }; localStorage.setItem(AREA_KEY, JSON.stringify(next)); setArea(next); setReportLocation(next) }
  ...
  return <>
    ...
    <main className="dashboard">
      <aside className="sidebar">
        ...
        <section className="composer"><div className="section-title"><h2>Report an issue</h2><span>Near {area.label}</span></div><form onSubmit={submit}><input value={text} onChange={event => setText(event.target.value)} maxLength={240} placeholder="What's happening? e.g. Metro stuck near Kaloor" aria-label="Describe a civic issue" /><p className="pin-status">📍 Pin: {reportLocation.label} · {reportLocation.lat.toFixed(4)}, {reportLocation.lng.toFixed(4)}</p><label className="upload">⌁ {image ? image.name : 'Add a photo (optional)'}<input type="file" accept="image/*" onChange={event => setImage(event.target.files?.[0] ?? null)} /></label><button>Submit report</button><button type="button" className="secondary" onClick={changeArea}>Change area</button></form></section>
      ...
      <section className="map-panel"><Map incidents={open} area={area} reportLocation={reportLocation} onPick={setReportLocation} onSelect={id => setSelected(incidents.find(incident => incident.id === id) ?? null)} /></section>
    </main>
