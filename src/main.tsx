...
function App() {
  ...
  useEffect(() => { if (!navigator.geolocation) return setLocationError('This browser cannot provide your location.'); navigator.geolocation.getCurrentPosition(position => { const next = { label: 'Your current location', lat: position.coords.latitude, lng: position.coords.longitude }; localStorage.setItem(AREA_KEY, JSON.stringify(next)); setArea(next); setLocationReady(true) }, () => setLocationError('Location permission is required to submit a report.'), { enableHighAccuracy: true, timeout: 10_000, maximumAge: 60_000 }) }, [])
  ...
  return <>
    ...
    <main className="dashboard">
      <aside className="sidebar">
        ...
        <section className="composer"><div className="section-title"><h2>Report an issue</h2><span>{locationReady ? 'GPS location ready' : 'Finding your location'}</span></div><form onSubmit={submit}><input value={text} onChange={event => setText(event.target.value)} maxLength={240} placeholder="What's happening? e.g. Metro stuck near Kaloor" aria-label="Describe a civic issue" /><p className="pin-status">📍 {locationError ?? (locationReady ? `Using your current GPS location · ${area.lat.toFixed(4)}, ${area.lng.toFixed(4)}` : 'Waiting for location permission…')}</p><label className="upload">⌁ {image ? image.name : 'Add a photo (optional)'}<input type="file" accept="image/*" onChange={event => setImage(event.target.files?.[0] ?? null)} /></label><button disabled={!locationReady}>Submit report</button></form></section>
