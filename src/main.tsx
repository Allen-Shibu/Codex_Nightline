  ...
  return <>
    ...
    <main className="dashboard">
      <aside className="sidebar">
        ...
        <form onSubmit={submit}><input value={text} onChange={event => setText(event.target.value)} maxLength={240} placeholder="What’s happening? e.g. Metro stuck near Kaloor" aria-label="Describe a civic issue" /><input type="file" accept="image/*" aria-label="Attach a photo" onChange={event => setImage(event.target.files?.[0] ?? null)} /><button>Report</button><button type="button" className="secondary" onClick={changeArea}>Change area</button></form>
        <div className="section-title"><h2>Open incidents</h2><span className="meta">{area.label}</span></div>
        <section className="feed">{open.map(incident => <article className="card incident" key={incident.id} onClick={() => setSelected(incident)}><span className={`dot ${severity(incident.reportCount)}`} /><div><h3>{incident.category} · {incident.location.label}</h3><p>{incident.description}</p>{incident.imageUrl && <img className="incident-image" src={incident.imageUrl} alt="Report evidence" />}<p className="meta">{age(incident.firstReported)} · {severity(incident.reportCount)}</p></div><span className="count">{'│'.repeat(Math.min(incident.reportCount, 8))} {incident.reportCount}</span></article>)}</section>
      </aside>
